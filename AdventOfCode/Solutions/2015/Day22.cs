    ...
    class Day22
    {
        ...
        public int Solution1(string input)
        {
            ...
            while (boundary.Any())
            {
                ...
                newCurrents = newCurrents.Where(it => it != null).ToList();
                newCurrents = newCurrents.Select(it => ApplyEffects(it)).ToList();
                var defeatedBoss = newCurrents.Where(it => it.BossHp <= 0).ToList();
                foreach(var item in defeatedBoss)
                {
                    if (item.SpentMana < minSpentMana)
                        minSpentMana = item.SpentMana;
                }
                newCurrents = newCurrents.Where(it => it.BossHp > 0).ToList();

                // perform boss move, If it defeats player, continue
                newCurrents = newCurrents.Select(it => ApplyBossAttack(it, bossValues[1])).ToList();
                newCurrents = newCurrents.Where(it => it.Hp > 0).ToList();
