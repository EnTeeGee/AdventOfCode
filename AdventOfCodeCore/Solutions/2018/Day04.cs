...
namespace AdventOfCodeCore.Solutions._2018
{
    internal class Day04
    {
        ...
        {
            var grouped = GetGroupedHistories(input);
            var sleepiest = grouped
                .Where(it => it.Sleeps.Any())
                .Select(it => new { it.Id, Info = it.GetMostProlificMinuteAndTimes() })
                .OrderByDescending(it => it.Info.Times)
                .First();
        ...
        {
            var items = Parser.ToArrayOf(input, it => new GuardEvent(it)).OrderBy(it => it.DateTime);
